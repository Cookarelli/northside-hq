...
import {productionStatuses,type Deliverable,type HqRecord,type Project} from '@/lib/hq-model';
...
export function HqToday({records,projects,posts,campaigns=[],loading,failed}:{records:HqRecord<Deliverable>[];projects:HqRecord<Project>[];posts:CalendarPost[];campaigns?:CampaignRecord[];loading:boolean;failed:boolean}) {
 ...
 const mine=open.filter(({id,data:d})=>[d.owner,d.publisher,...d.contributors,...(d.blocked?[d.blockedBy]:[])].includes(context.staffId)||reviewIds.has(id));
 const list=(items:HqRecord<Deliverable>[])=>items.length?<ul className="hq-deliverables">{items.map(({id,data:d})=>{const p=parent(d),color=p?projectColor(p,name):ownerColor(d.owner,name(d.owner));return <li key={id} className="hq-owner-row" style={{borderLeftColor:color.accent}}><div><Link href={'/projects/work/'+id}>{d.title}</Link><p>{p?.title||'Standalone'} · Owner: {name(d.owner)}</p><p>Assigned: {[...new Set([d.owner,d.publisher,...d.contributors].filter(Boolean))].map(name).join(', ')||'Unassigned'}</p><p>{d.productionDue?calendarDay(d.productionDue)+' · '+calendarTime(d.productionDue):'Due date not set'}</p>{d.blocked&&<p>Blocked · {d.blockedReason}</p>}{reviewIds.has(id)&&<p>Needs your approval</p>}</div><span className="tag">{productionStatuses[d.status]}</span></li>;})}</ul>:<p className="muted">No work in this section.</p>;
 ...
 <section className="panel"><h2>My Assignments</h2>{list(mine)}</section>
