...
export function HqRecentActivity({staff}:{staff:Staff[]}) {
 ...
 return <section className="panel"><h2>Recent Activity</h2>{error?<p role="alert">Activity unavailable. <Button variant="outline" onClick={()=>setRetry(n=>n+1)}>Retry activity</Button></p>:items?<ul className="hq-discussion">{items.map(item=><li key={item.id}><Link href={(item.kind==='project'?'/projects/':item.kind==='deliverable'?'/projects/work/':'/requests/')+encodeURIComponent(item.record_id)}>{item.snapshot.title||'Open '+item.kind}</Link><p>{personName(item.actor,staff)} · {item.action.replaceAll('-',' ').replaceAll('_',' ')}</p><time>{recordedTime(item.created_at)}</time></li>)}</ul>:<p role="status">Loading activity…</p>}{items?.length===0&&<p className="muted">No activity recorded yet.</p>}</section>;
}
