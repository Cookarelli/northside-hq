...
export function StoreOpenChecklist() {
 ...
 <StoreOpeningMilestone><div className="button-row"><Button disabled={!c||!!editor||busy} onClick={()=>edit({kind:'deliverable'})}>Add Item</Button><Button variant="ghost" disabled={!c||!!editor||busy} onClick={()=>edit({kind:'project'})}>Add Project</Button></div></StoreOpeningMilestone>
 ...
 <section className="store-open-progress" aria-label="Checklist progress"><div><span>{model.completed} of {model.items.length} complete</span><strong>{model.percent}%</strong></div><progress aria-label="Overall completion" max={100} value={model.percent}/><p className="hq-meta">{model.open} open items</p></section>
