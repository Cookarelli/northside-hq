...
import {STORE_OPEN_CHECKLIST_HREF,storeHasOpened} from '@/lib/store-opening';
import {calendarDay,calendarTime} from '@/lib/content-calendar';

export function StoreOpenChecklist() {
 const now=useHqClock(),opened=storeHasOpened(now);
 ...
 const model=checklistWork(workspace?.records||[],{includeArchivedProjects:opened}),c=workspace?.context,name=(id:string)=>c?.staff.find(s=>s.id===id)?.name||id||'Unassigned';
 ...
 <StoreOpeningMilestone>{!opened&&<div className="button-row"><Button disabled={!c||!!editor||busy} onClick={()=>edit({kind:'project'})}>Add Project</Button><Button variant="outline" disabled={!c||!!editor||busy} onClick={()=>edit({kind:'deliverable'})}>Add Deliverable</Button></div>}</StoreOpeningMilestone>
 ...
 {opened?<section className="panel store-open-summary" aria-label="Launch summary"><h3>Launch summary</h3><dl>{[['Total projects',model.summary.totalProjects],['Completed projects',model.summary.completedProjects],['Total deliverables',model.summary.totalDeliverables],['Completed deliverables',model.summary.completedDeliverables],['Remaining items',model.summary.remaining]].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section>:<section className="panel store-open-progress" aria-label="Checklist progress"><div><span>Overall completion</span><strong>{model.percent}%</strong><progress aria-label="Overall completion" max={100} value={model.percent}/><small>{model.items.length} projects and deliverables</small></div><div><span>Open items</span><strong>{model.open}</strong></div><div><span>Completed items</span><strong>{model.completed}</strong></div></section>}
 ...
 <div className="store-open-toolbar"><nav aria-label="Checklist views" className="button-row">{[['active',opened?'Remaining':'Active'],['departments','By Department'],['completed','Completed']].map(([id,label])=><Link key={id} scroll={false} aria-current={view===id?'page':undefined} className={'store-open-view'+(view===id?' is-active':'')} href={STORE_OPEN_CHECKLIST_HREF+'&view='+id} onClick={()=>setPage(0)}>{label}</Link>)}</nav><Link className="hq-meta" href={STORE_OPEN_CHECKLIST_HREF+'&view=plan'}>Opening strategy &amp; budget →</Link></div>
 ...
 <h3 ref={heading} tabIndex={-1} className="sr-only">{view==='completed'?'Completed':opened?'Remaining':'Active'} checklist work</h3>
 <p className="hq-meta">{view==='completed'?'Completed projects and deliverables.':opened?'Remaining launch work. Owners and assignees can continue updating these items.':'Work to finish before opening. Missing deadlines and dates after opening are flagged.'}</p>
 {!filtered.length&&<p className="panel empty-small">{query?'No checklist work matches your search.':view==='completed'?'No completed items yet.':model.items.length?'All checklist items are complete.':opened?'No launch work was recorded. The opening strategy and budget remain available.':'Add a project or a deliverable to start the store opening checklist.'}</p>}
 {[...groups.entries()].sort(([a],[b])=>view==='departments'?a.localeCompare(b):0).map(([label,items])=><section key={label} className="panel store-open-group" aria-label={label}><h3>{label}</h3><ul className="hq-work-list">{items.map(item=>{const timing=!opened&&!item.complete&&checklistTiming(item);return item.kind==='deliverable'?<HqWorkItem key={'deliverable-'+item.record.id} record={item.record} now={now} project={item.parent} context={c!} act={act} busy={busy||!!editor} statusControl assets={assets} notice={[timing,item.parent&&['completed','archived'].includes(item.parent.data.status)&&!item.complete?'Reopen the project to update this deliverable.':''].filter(Boolean).join(' · ')||undefined} onEdit={item.record.data.workflow==='task'&&canManageTask(item.record.data,item.parent?.data,c!)&&!['completed','archived'].includes(item.parent?.data.status||'')?()=>edit({kind:'deliverable',record:item.record}):undefined}/>:<li key={'project-'+item.record.id} className="hq-work-item"><div className="hq-work-copy"><Link className="hq-work-title" href={'/projects/'+item.record.id+'?tab=overview'}>{item.record.data.title}</Link><p className="hq-meta">Project · {item.department||'Owner'} · {name(item.owner)}</p><p className="hq-work-date">{item.due?calendarDay(item.due)+' · '+calendarTime(item.due):'No deadline'}<ChecklistDeadline date={item.due} complete={item.complete} now={now}/>{timing&&<strong> · {timing}</strong>}</p><Link href={'/projects/'+item.record.id+'?tab=deliverables'}>Open deliverables →</Link></div><div className="hq-work-actions"><span className="tag">{projectStatuses[item.record.data.status]}</span><span className="hq-meta">{taskPriorities[item.record.data.priority||'normal']} priority</span><div className="button-row">{!opened&&!item.complete&&item.record.data.status!=='archived'&&<Button disabled={busy||!!editor} variant="outline" onClick={()=>edit({kind:'deliverable',projectId:item.record.id})}>Add deliverable</Button>}{(c!.admin||item.owner===c!.staffId)&&<Button disabled={busy||!!editor} variant="outline" aria-label={'Edit project: '+item.record.data.title} onClick={()=>edit({kind:'project',record:item.record})}>Edit project</Button>}</div></div></li>;})}</ul></section>)}
