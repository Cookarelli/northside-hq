...
import {materialRoles,type MaterialRole} from '@/lib/hq-model';
import {supportedAssetTypes} from '@/lib/asset-policy';
import {uploadAsset,type UploadTicket} from '@/lib/asset-upload';
export type Asset={id:string;data:{name:string;type?:string}};
...
export function MaterialsEditor({value,onChange,available,onPendingChange,referenceOnly=false}:{value:Materials;onChange:(patch:Materials)=>void;available:Asset[];onPendingChange:(pending:boolean)=>void;referenceOnly?:boolean}) {
 ...
 <p className="muted">Private to your workspace. JPG, PNG, WebP, MP4, MOV or WebM · up to 40 MB. Use an external link for other formats.</p>
 <label className="field"><span>{referenceOnly?'Upload a supporting file':'Upload a file (reference, draft or final)'}</span><Input type="file" accept={supportedAssetTypes.join(',')} disabled={uploading||!!file} onChange={e=>{const selected=e.target.files?.[0];if(selected){ticket.current=undefined;setFile(selected);void upload(selected);}e.target.value='';}}/></label>
