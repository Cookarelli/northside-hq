...
export function HqWorkspace({view='list',id,area='projects'}:{view?:'list'|'project'|'deliverable';id?:string;area?:string}) {
  ...
  return <div className="hq-records">
    ...
    {view==='project' &&(project?<>
      ...
      {projectTab==='assets'&&<section className="panel"><div className="section-title"><h2>Project files and links</h2><Link href="/assets">All Assets →</Link></div><ResourceLinks {...project.data} available={assets}/>{!project.data.assets.length&&!project.data.references.length&&<p className="hq-meta">No files or links attached to the project yet. Use Edit project in Overview to add them.</p>}{deliverables.filter(d=>d.data.projectId===project.id&&!d.data.deletedAt&&(d.data.assets.length||d.data.references.length)).map(d=><div key={d.id}><h3>{d.data.title}</h3><ResourceLinks {...d.data} available={assets}/></div>)}</section>}
      {projectTab==='budget'&&<>
