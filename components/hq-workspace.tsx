...
export function HqWorkspace({view='list',id}:{view?:'list'|'project'|'deliverable';id?:string}) {
  ...
  return <div className="hq-records">
    ...
    {view==='list'&&<>
      <div className="section-title"><div><h2>Projects</h2><p className="muted">Scope the work, assign production and review each destination.</p></div><div className="button-row"><Button onClick={()=>setCreate('project')}>Add project</Button><Button variant="outline" onClick={()=>setCreate('deliverable')}>New standalone work</Button></div></div>
      ...
      <ProjectList projects={projects} deliverables={deliverables} name={name}/>
      <section className="panel"><h2>All deliverables</h2><DeliverableList records={deliverables} projects={projects} context={c} act={act} busy={busy}/></section>
      <LegacyAdoption records={records} context={c} act={act} busy={busy}/><EditorialHandoff records={records} context={c} act={act} busy={busy}/>
    ...
    {view==='project' &&(project?<>
      <section className="panel hq-owner-row hq-project-header" style={{borderLeftColor:projectColor(project.data,name).accent}}><p className="hq-meta">{projectTypes[project.data.type]}</p><h2>{project.data.title}</h2><div className="hq-header-meta"><span>Owner: <strong>{primaryOwnerLabel(project.data.owner,c.staff)}</strong></span><span className="tag">{projectStatuses[project.data.status]}</span><span>Due: {dateLabel(project.data.eventAt||project.data.auctionClosesAt)}</span></div></section>
      ...
      <section className="panel hq-project-people"><h2>People</h2><div className="two-fields"><div><p className="hq-meta">Primary owner</p><strong>{primaryOwnerLabel(project.data.owner,c.staff)}</strong></div><div><p className="hq-meta">Assigned staff</p><p>{project.data.members.map(name).join(', ')||'No staff assigned yet. Use Edit project to add people.'}</p></div></div></section>
      <section className="panel"><div className="section-title"><h2>Deliverables</h2>{eligibleProjects.some(p=>p.id===project.id)&&<div className="button-row"><Button onClick={()=>setCreate('task')}>Add deliverable</Button><Button variant="outline" onClick={()=>setCreate('deliverable')}>Add publishing work</Button></div>}</div>
        ...
        <HqProjectDeliverables records={deliverables.filter(d=>d.data.projectId===project.id)} project={project} context={c} act={act} busy={busy}/>
      </section>
      <section className="panel"><h2>Project information</h2><p className="hq-preserve-text">{project.data.brief||'No description yet.'}</p>{project.data.auctionOpensAt&&<p>Starts: {dateLabel(project.data.auctionOpensAt)}</p>}<Missing items={projectMissing(project.data)}/><ResourceLinks {...project.data} available={assets}/></section>
      <details className="panel hq-details"><summary>Budget and spending</summary><BudgetPanel project={project} context={c} act={act} busy={busy} name={name}/><HqSpending project={project} context={c}/></details>
