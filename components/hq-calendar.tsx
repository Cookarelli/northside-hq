...
import {STORE_OPENING_TIME_LABEL,STORE_OPENING,STORE_OPEN_CHECKLIST} from '@/lib/store-opening';
...
export function HqCalendar({records,projects,posts,campaigns=[],staff}:{records:HqRecord<Deliverable>[];projects:HqRecord<Project>[];posts:CalendarPost[];campaigns?:CampaignRecord[];staff:Staff[]}) {
 ...
 if(now===null)return <section className="panel"><h2 className="sr-only">Calendar view</h2><p role="status">Loading calendar…</p></section>;
 ...
 const colorFor=(e:typeof entries[number])=>e.milestone?{accent:'var(--primary)'}:e.deliverableOwnerColor?ownerColor(e.owner,name(e.owner)):e.projectData?projectColor(e.projectData,name):ownerColor(e.owner,name(e.owner),e.consignment);
 ...
 const item=(e:typeof entries[number],detail=false)=><li key={e.key}><Link className={'hq-calendar-item'+(e.milestone?' is-milestone':'')} href={e.href} style={{'--hq-calendar-owner':colorFor(e).accent} as CSSProperties}><strong>{e.title}</strong>{e.campaign&&<span className="hq-calendar-project">{e.campaign}</span>}{e.project!==e.title&&<span className="hq-calendar-project">{e.project}</span>}<span className="hq-calendar-meta">{!e.milestone&&primaryOwnerLabel(e.owner,staff)+' · '}<time dateTime={e.dateOnly?e.date.slice(0,10):e.date}>{e.milestone?STORE_OPENING_TIME_LABEL:e.dateOnly?'Time not announced':calendarTime(e.date)}</time>{e.milestone&&' · '+STORE_OPENING.timeZone}</span>{e.checklist&&<><span className="hq-calendar-meta">{e.project!==STORE_OPEN_CHECKLIST?STORE_OPEN_CHECKLIST:''}</span><ChecklistDeadline date={e.date} complete={!!e.complete} now={now}/></>}{detail&&<>{!e.milestone&&<span>Assigned: {e.assigned.map(name).join(', ')||'Unassigned'}</span>}<span>{e.status}{e.endAt?' · Ends '+calendarDay(e.endAt)+' '+calendarTime(e.endAt):''}</span></>}</Link></li>;
 return <section className="panel hq-visual-calendar" aria-label="Calendar"><div className="section-title"><h2 className="sr-only">Calendar view</h2><div className="button-row hq-view-switch" role="group" aria-label="Calendar view">{(['month','week','day'] as const).map(v=><Button key={v} variant="outline" aria-pressed={view===v} onClick={()=>setView(v)}>{v[0].toUpperCase()+v.slice(1)}</Button>)}</div></div>
