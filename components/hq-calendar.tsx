...
export function HqCalendar({records,projects,posts,campaigns=[],staff}:{records:HqRecord<Deliverable>[];projects:HqRecord<Project>[];posts:CalendarPost[];campaigns?:CampaignRecord[];staff:Staff[]}) {
 ...
 const colorFor=(e:typeof entries[number])=>e.deliverableOwnerColor?ownerColor(e.owner,name(e.owner)):e.projectData?projectColor(e.projectData,name):ownerColor(e.owner,name(e.owner),e.consignment);
 ...
 const item=(e:typeof entries[number],detail=false)=><li key={e.key}><Link className="hq-calendar-item" href={e.href} style={{borderLeftColor:colorFor(e).accent}}><strong>{e.title}</strong>{e.campaign&&<span className="hq-calendar-project">{e.campaign}</span>}{e.project!==e.title&&<span className="hq-calendar-project">{e.project}</span>}<span className="hq-calendar-meta">{primaryOwnerLabel(e.owner,staff)} · <time dateTime={e.dateOnly?e.date.slice(0,10):e.date}>{e.dateOnly?'Time not announced':calendarTime(e.date)}</time></span>{detail&&<><span>Assigned: {e.assigned.map(name).join(', ')||'Unassigned'}</span><span>{e.status}{e.endAt?' · Ends '+calendarDay(e.endAt)+' '+calendarTime(e.endAt):''}</span></>}</Link></li>;
