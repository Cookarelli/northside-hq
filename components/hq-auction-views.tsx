...
import {canManageAuction,nextAuctionNumber,type AuctionCampaignData} from '@/lib/auction-campaigns';
...
export function AuctionViews({area,current,history,campaigns,records,project,context,assets,act,busy,selected}:{area:'current-auction'|'auction-history'|'budget';current?:HqRecord<AuctionCampaignData>;history:HqRecord<AuctionCampaignData>[];campaigns:HqRecord<AuctionCampaignData>[];records:HqRecord<Deliverable>[];project:HqRecord<Project>;context:HqContext;assets:Asset[];act:Action;busy:boolean;selected:string}){
 ...
 return <section className="panel"><div className="section-title"><h2>{area==='current-auction'?'Current Auction':area==='auction-history'?'Auction History':'Budget & Reconciliation'}</h2>{area==='current-auction'&&canManageAuction(project.data,context)&&!['archived','completed'].includes(project.data.status)&&<Button disabled={busy} onClick={()=>setAdding(!adding)}>{adding?'Cancel new auction':'Add Auction Campaign'}</Button>}</div>
 ...
 {area!=='current-auction'&&<ul className="hq-auction-index" aria-label="Auctions">{[...choices].sort((a,b)=>b.data.auction_number-a.data.auction_number).map(c=><li key={c.id}><Link href={projectTabHref(project.id,area,{auction:c.id})} aria-current={visible?.id===c.id?'true':undefined}>#{c.data.auction_number} — {c.data.name}</Link><span className="hq-meta">{calendarDay(c.data.closesAt)} · {c.data.reconciliation?.at?'Reconciled':'Open'}</span></li>)}</ul>}
