...
export function HqProjectCard({project,deliverables,name,now=null}:{project:HqRecord<Project>;deliverables:HqRecord<Deliverable>[];name:(id:string)=>string;now?:number|null}) {
 ...
 return <article className="panel hq-project-card" style={{borderLeftColor:projectColor(p,name).accent}}><h3><Link href={'/projects/'+project.id} title={p.title}>{p.title}</Link></h3><div className="hq-card-owner"><span>Owner: <strong>{primaryOwnerName(p.owner,name)}</strong></span><span className="tag">{projectStatuses[p.status]}</span></div><div className="hq-card-next"><span className="hq-meta">Next deliverable</span>{next?<Link href={deliverableHref(next.id,next.data)} title={next.data.title}>{next.data.title}</Link>:<span>No open deliverables</span>}</div><p className="hq-card-date">{date?calendarDay(date)+' · '+calendarTime(date):'No due date'}<span className="hq-urgency">{(p.storeOpenChecklist?checklistDueState:dueState)(date,p.status==='completed',now)}</span></p><div className="hq-card-staff"><span className="hq-meta">Assigned staff</span><p>{assigned.map(name).join(', ')||'No staff assigned'}</p></div>{!!work.length&&<label className="hq-project-progress"><span>{completed} of {work.length} complete</span><progress value={completed} max={work.length}/></label>}</article>;
}
