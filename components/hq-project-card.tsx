...
export function HqProjectCard({project,deliverables,name}:{project:HqRecord<Project>;deliverables:HqRecord<Deliverable>[];name:(id:string)=>string}) {
 const p=project.data,work=deliverables.filter(d=>d.data.projectId===project.id),next=work.filter(d=>!finished(d.data)).sort((a,b)=>(a.data.productionDue||'9999').localeCompare(b.data.productionDue||'9999'))[0];
 ...
 const assigned=[...new Set([...p.members,...work.flatMap(d=>[d.data.owner,d.data.publisher,...d.data.contributors])].filter(Boolean))];
 return <article className="panel hq-project-card" style={{borderLeftColor:projectColor(p,name).accent}}><h3><Link href={'/projects/'+project.id}>{p.title}</Link></h3><p>Owner: {name(p.owner)}</p><p>Assigned: {assigned.map(name).join(', ')||'Unassigned'}</p><p>Next: {next?<Link href={'/projects/work/'+next.id}>{next.data.title}</Link>:'No open deliverables'}</p><p>Due: {date?calendarDay(date)+' · '+calendarTime(date):'Not set'}</p><span className="tag">{projectStatuses[p.status]}</span>{!!work.length&&<label className="hq-project-progress"><span>{completed} / {work.length} deliverables complete</span><progress value={completed} max={work.length}/></label>}</article>;
}
