'use client';
import {auctionRecordText} from '@/lib/auction-campaigns';
...
type Inbox={items:Notice[];unread:number;nextOffset:number|null;background?:{active:boolean;lastCheckedAt:string|null;emailEnabled:false}|null};
export function HqNotifications(){
 ...
 {open&&<section id="hq-notification-inbox" className="panel hq-inbox" aria-label="My notifications"><div className="section-title"><h2>My notifications</h2><Button variant="outline" onClick={()=>setOpen(false)}>Close notifications</Button></div><p className="muted">{inbox?.background?.active?'Deadline reminders are checked automatically every five minutes, even while HQ is closed.':'Automatic reminder checks are not confirmed. Use Check reminders to refresh your inbox.'} Email notifications are off.</p><Button variant="outline" disabled={busy} onClick={()=>void refresh()}>Check reminders</Button>{checked&&<p className="muted">Last checked {recordedTime(checked)}</p>}{error&&<p role="alert">Notifications unavailable: {error}</p>}{!inbox&&!error&&<p role="status">Loading notifications…</p>}
 <ul className="hq-discussion">{inbox?.items.map(n=><li key={n.id}><Link href={recordActionHref(n.kind,n.record_id,n.metadata.comment?'comment':n.category)}>{auctionRecordText(n.kind==='deliverable'?n.message.replace(/^Assigned as owner: /,'Assigned to you: '):n.message,n.record_id)}</Link><time>{recordedTime(n.created_at)}</time>{n.resolved_at&&<p className="muted">No longer due for action</p>}{n.metadata.comment&&<p>{n.metadata.comment}</p>}<Button variant="outline" disabled={busy} onClick={()=>void mark(n)}>{n.read_at?'Mark unread':'Mark read'}</Button></li>)}</ul>
