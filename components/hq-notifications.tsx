...
export function HqNotifications(){
 ...
 return <div className="hq-notifications"><Button variant="outline" aria-expanded={open} aria-controls="hq-notification-inbox" onClick={()=>{setOpen(!open);if(!open)void refresh();}}><Bell size={17} aria-hidden="true"/>Notifications{inbox?' ('+inbox.unread+' unread)':error?' · unavailable':''}</Button>
 ...
 <ul className="hq-discussion">{inbox?.items.map(n=><li key={n.id}><Link href={(n.kind==='project'?'/projects/':n.kind==='deliverable'?'/projects/work/':'/requests/')+encodeURIComponent(n.record_id)}>{n.message}</Link><time>{recordedTime(n.created_at)}</time>{n.resolved_at&&<p className="muted">No longer due for action</p>}{n.metadata.comment&&<p>{n.metadata.comment}</p>}<Button variant="outline" disabled={busy} onClick={()=>void mark(n)}>{n.read_at?'Mark unread':'Mark read'}</Button></li>)}</ul>
