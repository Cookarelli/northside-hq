...
export function HqNotifications(){
 ...
 <ul className="hq-discussion">{inbox?.items.map(n=><li key={n.id}><Link href={recordActionHref(n.kind,n.record_id,n.metadata.comment?'comment':n.category)}>{n.kind==='deliverable'?n.message.replace(/^Assigned as owner: /,'Assigned to you: '):n.message}</Link><time>{recordedTime(n.created_at)}</time>{n.resolved_at&&<p className="muted">No longer due for action</p>}{n.metadata.comment&&<p>{n.metadata.comment}</p>}<Button variant="outline" disabled={busy} onClick={()=>void mark(n)}>{n.read_at?'Mark unread':'Mark read'}</Button></li>)}</ul>
