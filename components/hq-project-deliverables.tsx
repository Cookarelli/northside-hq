...
export function HqProjectDeliverables({records,project,context,act,busy,assets=[],campaigns=[],focused=''}:{records:HqRecord<Deliverable>[];project:HqRecord<Project>;context:HqContext;act:Action;busy:boolean;assets?:Asset[];campaigns?:HqRecord<AuctionCampaignData>[];focused?:string}){
 ...
 const row=(r:HqRecord<Deliverable>)=><HqWorkItem key={r.id} focused={focused===r.id} assets={assets} record={r} now={now} project={project} context={context} act={act} busy={busy} statusControl onEdit={r.data.workflow==='task'&&canManageTask(r.data,project.data,context)&&!['completed','archived'].includes(project.data.status)?()=>setEditing(r.id):undefined}/>;
 const record=visible.find(r=>r.id===editing);
 return <>{groups.map(group=><AuctionCampaign key={group.key} campaign={campaigns.find(c=>c.id===group.key)} focused={focused} name={group.name} records={group.records} project={project} context={context} assets={assets} act={act} busy={busy}/>)}{record&&<ProjectTaskForm key={record.id+':'+record.data.version} project={project} assets={assets} record={record} context={context} busy={busy} onSave={act} onCancel={()=>setEditing('')}/>}{(!groups.length||other.length>0)&&<h3>{groups.length?'Other auction work':'Open deliverables'} <span className="hq-count">{open.length}</span></h3>}{open.length?<ul className="hq-work-list">{open.map(row)}</ul>:!groups.length?<p className="hq-meta">No open deliverables. Add one above to schedule the next step.</p>:null}{!!complete.length&&<details className="hq-details" open={complete.some(r=>r.id===focused)||undefined}><summary>Completed ({complete.length})</summary><ul className="hq-work-list">{complete.map(row)}</ul></details>}</>;
}
