...
import {budgetVariance,parseUsd,usd,usdInput,varianceLabel} from '@/lib/deliverable-budget';
...
export function BudgetFields({planned,actual,onPlanned,onActual,disabled=false,actualFromEntries=false}:{actualFromEntries?:boolean;planned:string;actual:string;onPlanned:(v:string)=>void;onActual:(v:string)=>void;disabled?:boolean}){return <fieldset className="campaign-fields hq-budget-fields" disabled={disabled}><legend>Deliverable budget · USD</legend><div className="two-fields"><label className="field"><span>Planned Budget (USD)</span><Input inputMode="decimal" placeholder="150.00" maxLength={13} value={planned} onChange={e=>onPlanned(e.target.value)}/></label><label className="field"><span>Actual Spend (USD, optional)</span><Input readOnly={actualFromEntries} inputMode="decimal" placeholder="Not recorded" maxLength={13} value={actual} onChange={e=>onActual(e.target.value)}/></label></div><p className="hq-meta">{actualFromEntries?'Use Spend by channel in Budget to record actuals.':'Actual spend can stay blank until spending occurs. It is not required for publication.'}</p></fieldset>;}
export function DeliverableBudget({record,project,context,busy,act,compact=false}:{compact?:boolean;record:HqRecord<Deliverable>;project?:Project;context:HqContext;busy:boolean;act:Action}){
 const d=record.data,[editing,setEditing]=useState(false);
 if(d.deletedAt)return null;
 return <section className={compact?'hq-deliverable-budget':'panel'}>{compact?<h5>Deliverable budget</h5>:<h2>Deliverable budget</h2>}<BudgetSummary deliverable={d}/>{canWork(d,project,context)&&(editing?<BudgetEditor key={d.version} record={record} act={act} busy={busy} onClose={()=>setEditing(false)}/>:<Button variant="outline" onClick={()=>setEditing(true)}>Edit budget and spend</Button>)}</section>;
}
