...
export function BudgetRollup({records,label,campaignBudget}:{records:HqRecord<Deliverable>[];label:string;campaignBudget?:number|null}){
 const t=budgetTotals(records),remaining=campaignBudget==null?null:BigInt(campaignBudget)-t.planned;
 return <section className="hq-budget-rollup" aria-label={label+' budget totals'}><h4>{label} · Budget totals</h4><dl className="hq-budget-values"><div><dt>Campaign Budget</dt><dd>{campaignBudget==null?'Not set':usd(campaignBudget)}</dd></div><div><dt>Allocated to Deliverables</dt><dd>{usd(t.planned)}</dd></div><div><dt>{remaining!==null&&remaining<BigInt(0)?'Overallocated':'Unallocated'}</dt><dd>{remaining===null?'Not available':usd(remaining<BigInt(0)?-remaining:remaining)}</dd></div></dl><dl className="hq-budget-values"><div><dt>Total Planned Budget</dt><dd>{usd(t.planned)}</dd></div><div><dt>Total Actual Spend</dt><dd>{t.pending===t.count?'Not recorded':usd(t.actual)}</dd></div><div><dt>Total Variance</dt><dd>{t.pending===t.count?'Not available':varianceLabel(t.variance)}</dd></div></dl>{(t.pending>0||t.unbudgeted>0)&&<p className="hq-meta">Based on recorded amounts. {t.unbudgeted} deliverable{t.unbudgeted===1?'':'s'} without a planned budget; {t.pending} without actual spend.</p>}</section>;
}
