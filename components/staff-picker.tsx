...
export function StaffPicker({label,value,onChange,staff,disabled=false}:{label:string;value:string[];onChange:(ids:string[])=>void;staff:Staff[];disabled?:boolean}) {
 ...
 <div className="hq-staff-options">{staff.filter(s=>s.name.toLowerCase().includes(search.toLowerCase())).map(s=><label key={s.id}><input type="checkbox" checked={value.includes(s.id)} onChange={e=>onChange(e.target.checked?[...value,s.id]:value.filter(id=>id!==s.id))}/>{s.name}</label>)}</div>
 </fieldset>;
}
