...
export function JonsContent({workspace,onSaved}:{workspace:Workspace;onSaved:(asset:SavedAsset)=>void}){
 ...
 return <div className="jon-content">
  <div className="section-title jon-header"><div><h2>Jon&apos;s Content</h2><p className="hq-meta">Upload content here, then assign it to a project or deliverable.</p></div><Button className="jon-upload-button" disabled={busy||files.length>0} onClick={()=>input.current?.click()}><Upload size={20}/>Upload Content</Button><input ref={input} hidden type="file" multiple accept={assetAccept} onChange={e=>{choose(Array.from(e.target.files||[]));e.target.value='';}}/></div>
