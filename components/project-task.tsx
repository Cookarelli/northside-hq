...
export function ProjectTaskForm({project,record,context,busy,onSave,onCancel}:{project:HqRecord<Project>;record?:HqRecord<Deliverable>;context:HqContext;busy:boolean;onSave:Action;onCancel:()=>void}) {
 const [id]=useState(()=>record?.id||clientId()),[error,setError]=useState('');
 const [data,setData]=useState<TaskInput>(()=>({title:record?.data.title||'',instructions:record?.data.instructions||'',projectId:project.id,assignees:record?taskAssignees(record.data):[context.staffId],productionDue:record?.data.productionDue||'',endAt:record?.data.endAt||'',priority:record?.data.priority||'normal',notes:record?.data.notes||''}));
 const update=(patch:Partial<TaskInput>)=>setData(old=>({...old,...patch}));
 const manager=canManageTask(record?.data,project.data,context);
 return <form className="panel hq-form" onSubmit={async e=>{e.preventDefault();const parsed=taskInput.safeParse(data);if(!parsed.success){setError(parsed.error.issues[0].message);return;}setError('');if(await onSave({action:'save-task',id,version:record?.data.version||0,data:parsed.data}))onCancel();}}><h3>{record?'Edit deliverable':'Add deliverable'}</h3><fieldset disabled={busy} className="campaign-fields">
 <label className="field"><span>Title</span><Input required maxLength={300} value={data.title} onChange={e=>update({title:e.target.value})}/></label>
 <label className="field"><span>Description</span><Textarea maxLength={12000} value={data.instructions} onChange={e=>update({instructions:e.target.value})}/></label>
 ...
 <label className="field"><span>End date and time (optional window, Central)</span><Input type="datetime-local" disabled={!data.productionDue} min={data.productionDue} value={data.endAt} onChange={e=>update({endAt:e.target.value})}/></label>
 ...
 {error&&<p role="alert">{error}</p>}<div className="button-row"><Button type="submit">{busy?'Saving…':record?'Save deliverable':'Create deliverable'}</Button><Button type="button" variant="outline" onClick={onCancel}>Cancel</Button></div>
 </fieldset></form>;
}
...
export function DeliverableDetails({record,project,context,busy,act}:{record:HqRecord<Deliverable>;project?:Project;context:HqContext;busy:boolean;act:Action}) {
 ...
 return <section className="panel"><h2>Priority and notes</h2>{d.completedAt&&<p>Completed {recordedTime(d.completedAt)}</p>}{manager?<form className="hq-form" onSubmit={e=>{e.preventDefault();void act({action:'task-metadata',id:record.id,version:d.version,data:{priority,notes}});}}><label className="field"><span>Priority</span><select disabled={busy} value={priority} onChange={e=>setPriority(e.target.value as TaskPriority)}>{Object.entries(taskPriorities).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></label><label className="field"><span>Optional notes</span><Textarea disabled={busy} maxLength={12000} value={notes} onChange={e=>setNotes(e.target.value)}/></label><Button disabled={busy} type="submit">Save priority and notes</Button></form>:<><p>{taskPriorities[d.priority||'normal']}</p><p className="hq-preserve-text">{d.notes||'No notes'}</p></>}
