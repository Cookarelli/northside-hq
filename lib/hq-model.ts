...
export const materials={assets:z.array(staffId).max(60),references:z.array(https).max(60),assetRoles:roleMap.default({}),linkRoles:roleMap.default({})};
const common = {title:z.string().trim().min(1, 'Add a title.').max(300),owner:staffId,...materials};
export const checklistFields={storeOpenChecklist:z.boolean().optional(),department:z.string().trim().max(100).optional(),priority:z.enum(['low','normal','high','urgent']).optional()};
export const projectInput = z.object({...common,...checklistFields,
...
export type Approval = {by:string;at:string;admin?:boolean;projectVersion:number|null;budgetVersion:number|null;reviewedVersion?:number;approvedVersion?:number;contentVersion?:number;comment?:string;package?:PublishingPackage};
...
export function projectDraft(p:Project):ProjectInput {
  ...
  return {...draft,...(p.storeOpenChecklist!==undefined?{storeOpenChecklist:p.storeOpenChecklist}:{}),...(p.department!==undefined?{department:p.department}:{}),...(p.priority!==undefined?{priority:p.priority}:{}),eventAt:scheduleWall(draft.eventAt),auctionOpensAt:scheduleWall(draft.auctionOpensAt),auctionClosesAt:scheduleWall(draft.auctionClosesAt)};
}
...
export function approvalCurrent(d:Deliverable,p?:Project) {
  return !!d.approval?.approvedVersion && d.approval.contentVersion===d.contentVersion && (d.approval.admin===true||[d.owner,d.publisher,d.campaignOwner,...(d.campaignAssignees||[]),...d.contributors,...(p?.members||[]),d.projectId?p?.owner:d.approver].includes(d.approval.by)) && (!d.projectId || (p?.status==='active' && (d.promotionMode!=='paid'||!!p.budget) && d.approval.projectVersion===p.version));
}
...
export function canWork(d:Deliverable,p:Project|undefined,c:HqContext) {return c.admin || d.campaignOwner===c.staffId || !!d.campaignAssignees?.includes(c.staffId) || d.owner===c.staffId || d.publisher===c.staffId || d.contributors.includes(c.staffId) || p?.owner===c.staffId || !!p?.members.includes(c.staffId) || (!d.projectId&&d.approver===c.staffId);}
