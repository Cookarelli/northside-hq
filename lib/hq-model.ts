...
export function approvalCurrent(d:Deliverable,p?:Project) {
  return !!d.approval?.approvedVersion && d.approval.contentVersion===d.contentVersion && (d.approval.admin===true||[d.owner,d.publisher,...d.contributors,...(p?.members||[]),d.projectId?p?.owner:d.approver].includes(d.approval.by)) && (!d.projectId || (p?.status==='active' && (d.promotionMode!=='paid'||!!p.budget) && d.approval.projectVersion===p.version));
}
