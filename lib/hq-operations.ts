...
export function instant(wall:string){try{return chicagoInstant(wall);}catch{return null;}}
...
export function actionable(d:Deliverable,p?:Project){return !finished(d)&&!['completed','archived'].includes(p?.status||'');}
export function nextAction(d:Deliverable,p?:Project){if(d.blocked)return 'Resolve: '+d.blockedReason;if(!d.owner)return 'Assign an accountable owner';if(d.status==='needs_review')return 'Review the submitted version';if(d.status==='ready'&&!approvalCurrent(d,p))return 'Renew owner approval';if(d.status==='ready')return d.publishing?'Publish or confirm each platform':'Mark this task Done';if(d.status==='to_do')return 'Start production';return 'Finish production and submit for review';}
export function personName(id:string,staff:Staff[]){return staff.find(p=>p.id===id)?.name||id||'Unassigned';}
export type ScheduleRow={key:string;id:string;kind:'deliverable'|'legacy';title:string;date:string;projectId:string;owner:string;publisher:string;contributors:string[];format:string;platform:string;status:string;productionStatus:string;stateLabel:string;ready:boolean;action:string;recurring?:boolean};
export function scheduleRows(records:HqRecord<Deliverable>[],projects:HqRecord<Project>[],mode:'publication'|'production',legacy:CalendarPost[]=[],from='',to=''):ScheduleRow[]{
 ...
 for(const {id,data:d} of records){const p=projects.find(p=>p.id===d.projectId)?.data,base={id,kind:'deliverable' as const,title:d.title,projectId:d.projectId,owner:d.owner,publisher:d.publisher||'',contributors:d.contributors,format:d.format,productionStatus:d.status,ready:d.status==='ready'&&approvalCurrent(d,p)&&!d.blocked,action:nextAction(d,p)};
  if(mode==='production'){rows.push({...base,key:id+':production',date:d.productionDue,platform:'',status:d.status,stateLabel:productionStatuses[d.status]});continue;}
