...
export function checklistWork(records:WorkspaceRecord[],{includeArchivedProjects=false}:{includeArchivedProjects?:boolean}={}) {
 ...
 const summary={totalProjects:items.filter(i=>i.kind==='project').length,completedProjects:items.filter(i=>i.kind==='project'&&i.complete).length,totalDeliverables:deliverables.length,completedDeliverables:deliverables.filter(d=>finished(d.data)).length,remaining:items.length-completed};
 return {projects,items,summary,completed,open:items.length-completed,percent:items.length?Math.round(completed/items.length*100):0};
}
