...
import {auctionFinanceCommand} from '@/lib/auction-finance';
...
export async function GET(request:Request) {
  try {
    ...
    if(kind==='auction-spend'&&id&&Number.isSafeInteger(offset)&&offset>=0){const {data,error}=await client.rpc('hub_auction_finance',{p_action:'list',p_payload:{deliverableId:id,offset}});if(error)throw error;return Response.json(data,{headers});}
...
export async function POST(request:Request) {
  try {
    ...
    const parsed=z.union([auctionFinanceCommand,createAuctionCampaignCommand,auctionCampaignCommand,deliverableBudgetCommand,auctionEditorCommand,taskCommand,hqCommand]).safeParse(input);
    if(!parsed.success) return Response.json({error:parsed.error.issues[0]?.message||'Check the fields.'},{status:400,headers});
    const {action,...payload}=parsed.data;
    const financeAction=({'auction-spend':'spend','auction-spend-reverse':'reverse','auction-reconcile':'reconcile'} as Record<string,string>)[action];
    const {data,error}=await (await db()).rpc(financeAction?'hub_auction_finance':action==='create-auction-campaign'?'hub_create_auction_campaign':action==='save-auction-campaign'?'hub_auction_campaign':action==='deliverable-budget'?'hub_deliverable_budget':action==='save-auction-deliverable'?'hub_auction_deliverable':['save-task','task-status','task-delete','task-restore','task-metadata'].includes(action)?'hub_project_tasks':action==='adopt-editorial'?'hub_hq_editorial':['reschedule','reminders','notification-read','spend','spend-reverse','comment'].includes(action)?'hub_hq_operations':'hub_hq',financeAction?{p_action:financeAction,p_payload:payload}:['adopt-editorial','save-auction-deliverable','save-auction-campaign','create-auction-campaign','deliverable-budget'].includes(action)?{p_payload:payload}:{p_action:action,p_payload:payload});
    if(error) {
