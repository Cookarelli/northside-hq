...
import {z} from 'zod';
import {STORE_OPEN_PLAN_ID,storeOpenPlan} from '@/lib/store-opening';
...
const validators={
 plan:z.object({budget:z.union([z.literal(1000),z.literal(2000),z.literal(3000),z.literal(5000)]),launchDate:z.string(),address:z.string().max(500),campaign:z.string().min(1).max(150),aov:z.number().positive(),margin:z.number().positive().max(100)}).passthrough(),
 post:postSchema,
