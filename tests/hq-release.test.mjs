...
test('request notes persist independently, retain author and line breaks, and deny unauthorized access',async()=>{
 await actor('maker');
 await hq('save-request',{id:'notes-request',version:0,data:{...blankRequest,title:'Test handoff',purpose:'Verify internal notes'}});
 const first={kind:'request',id:'notes-request',commentId:crypto.randomUUID(),body:'Waiting on assets.\nNext: confirm delivery.',mentions:[]};
 await ops('comment',{...first,actor:'admin',org_id:'other-org'});
 const select=()=>db.query("select id,actor,body,created_at from hq_comments where record_id='notes-request' order by created_at desc,id");
 assert.equal((await select()).rows[0].actor,'maker');
 await db.exec('reset role');await db.close();db=new PGlite(dir);await actor('maker');
 assert.equal((await select()).rows[0].body,first.body);
 await actor('owner');await ops('comment',{...first,commentId:crypto.randomUUID(),body:'Assets received.\nReady for handoff.'});
 const rows=(await select()).rows;assert.equal(rows.length,2);assert.ok(rows.some(row=>row.body===first.body));assert.ok(rows.every(row=>row.created_at));
 await actor('maker');await ops('comment',first);assert.equal((await select()).rows.length,2);
 await assert.rejects(db.query("update hq_comments set body='Changed' where record_id='notes-request'"),/permission denied/);
 for(const who of ['foreign','pending','signed-out']){
  await actor(who);assert.equal((await select()).rows.length,0);
  await assert.rejects(ops('comment',{...first,commentId:crypto.randomUUID()}));
 }
 await db.exec('reset role;set role anon');
 await assert.rejects(select(),/permission denied/);
 await assert.rejects(ops('comment',first),/permission denied/);
});
